...
function jtbc_cms_admin_manage_editdisp()
{
  ...
  if (file_exists($trootstr))
  {
    ...
    $tmpstr .= '<xml mode="' . $tmode . '" author="jetiben">' . CRLF;
    ...
    $tmpstr .= '  </configure>' . CRLF;
    $tmpstr .= '</xml>' . CRLF;
    if (file_put_contents($trootstr, $tmpstr)) jtbc_cms_admin_msg(ii_itake('global.lng_public.succeed', 'lng'), $tbackurl, 1);
    else jtbc_cms_admin_msg(ii_itake('global.lng_public.failed', 'lng'), $tbackurl, 1);
  }
  else jtbc_cms_admin_msg(ii_itake('manage.notexists', 'lng'), $tbackurl, 1);
}
...
function jtbc_cms_admin_manage_edit()
{
  ...
  else jtbc_cms_admin_msg(ii_itake('manage.notexists', 'lng'), $nuri, 1);
}
