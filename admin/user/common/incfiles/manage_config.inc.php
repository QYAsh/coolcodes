...
function pp_get_mymanage_module($strers)
{
  ...
  while($tentry = $twebdir -> read())
  {
    if (!(is_numeric(strpos($tentry, '.'))))
    {
      ...
      {
        if (ii_get_xrootatt($tfilename, 'mode') == 'jtbcfgf') $tmpstr .= pp_get_mymanage_module($tpath . $tentry . '/');
      }
    }
  }
  $twebdir -> close();
...
function jtbc_cms_admin_manage_adddisp()
{
  ...
  if ($trs)
  {
    jtbc_cms_admin_msg(ii_itake('global.lng_public.add_failed', 'lng'), $tbackurl, 1);
  }
  else
  {
...
function jtbc_cms_admin_manage_editdisp()
{
  ...
  {
    jtbc_cms_admin_msg(ii_itake('global.lng_public.edit_failed', 'lng'), $tbackurl, 1);
  }
}
...
function jtbc_cms_admin_manage_edit()
{
  ...
  {
    jtbc_cms_admin_msg(ii_itake('global.lng_public.not_exist', 'lng'), $tbackurl, 1);
  }
}
