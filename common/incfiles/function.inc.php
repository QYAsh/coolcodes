...
function ii_ctemplate(&$templatestr, $distinstr)
{
  if (is_numeric(strpos($templatestr, $distinstr)))
  {
    ...
    if (count($tarys) == 3)
    {
      $templatestr = $tarys[0] . JTBC_CINFO . $tarys[2];
      return $tarys[1];
    }
  }
}
...
{
  $ttypelist = '.asp.asa.aspx.bat.bmp.css.cfm.com.doc.db.dll.exe.fla.gif.htm.html.inc.ini.jpg.js.jtbc.log.mdb.mid.mp3.png.php.rm.rar.swf.txt.wav.xls.xml.zip';
  ...
  while($tentry = $twebdir -> read())
  {
    if (!(is_numeric(strpos($tentry, '.'))))
    {
      ...
      if (file_exists($tfilename))
      {
        ...
        $tarys += $tary;
        if (ii_get_xrootatt($tfilename, 'mode') == 'jtbcfgf') $tarys += ii_get_variable_config($path . $tentry . '/');
      }
    }
  }
  $twebdir -> close();
  ...
  while($tentry = $twebdir -> read())
  {
    if (!(is_numeric(strpos($tentry, '.'))))
    {
      ...
      if (file_exists($tfilename))
      {
        ...
        if (ii_get_xrootatt($tfilename, 'mode') == 'jtbcfgf') $tmpstr .= ii_get_myvalid_module($tpath . $tentry . '/');
      }
    }
  }
  $twebdir -> close();
