...
function jtbc_cms_admin_manage_adddisp()
{
  ...
  if (ii_isnull($tsort) || ii_isnull($tlng)) jtbc_cms_admin_msg(ii_itake('manage-sort.empty', 'lng'), $tbackurl, 1);
  ...
  else jtbc_cms_admin_msg(ii_itake('manage-sort.dbaseerror', 'lng'), $tbackurl, 1);
}
...
function jtbc_cms_admin_manage_deletedisp()
{
  ...
  if ($trs)
  {
    ...
    if ($tfidCount == 0)
    {
      ...
      if ($trs) jtbc_cms_admin_msg(ii_itake('manage-sort.deletesucceed', 'lng'), $tbackurl, 1);
      else jtbc_cms_admin_msg(ii_itake('manage-sort.deletefailed', 'lng'), $tbackurl, 1);
    }
    else jtbc_cms_admin_msg(ii_itake('manage-sort.delete_has', 'lng'), $tbackurl, 1);
  }
  else jtbc_cms_admin_msg(ii_itake('manage-sort.deleteerr', 'lng'), $tbackurl, 1);
}

function jtbc_cms_admin_manage_orderdisp()
{
  global $ngenre, $slng;
  jtbc_cms_admin_orderdisp($ngenre, 'sort', " and " . ii_cfname('lng') . "='$slng'");
}
...
function jtbc_cms_admin_manage_action()
{
  ...
  switch($_GET['action'])
  {
    ...
    case 'delete':
      ...
      break;
    case 'order':
      jtbc_cms_admin_manage_orderdisp();
      break;
    case 'control':
